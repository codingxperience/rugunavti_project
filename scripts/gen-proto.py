...
/* Mega-menus: take them out of the nav-item flow and let JS place + clamp them to
   the viewport (position-fixed), so they can be wide but never cross the edge. */
.rg .nav-links{position:static;}
.rg .nav-item{position:static;}
.rg .nav-item.has-mega::after{display:none;}
.rg .mega{position:fixed;top:118px;left:16px;transform:translateY(10px);width:min(1180px,calc(100vw - 24px));max-width:none;}
.rg .nav-item.has-mega:hover .mega{transform:translateY(0);}
.rg .mega::before{content:"";position:absolute;left:0;right:0;top:-18px;height:18px;}
/* 3D object illustration on each mega feature card */
.rg .mega-feature{overflow:hidden;align-items:center;text-align:left;}
.rg .mega-feature .mf-illus{display:block;width:auto;height:132px;margin:10px auto 6px;object-fit:contain;filter:drop-shadow(0 16px 22px rgba(17,17,17,0.22));}
"""
